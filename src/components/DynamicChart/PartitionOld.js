...
export class PartitionOld {
    ...
    partition() {
        ...
        function chart(s) {
            ...
            var colorTitle = (d) => {
                let r = ['#000000', "#777777", '#d3d3d3', "#eaeaea"];
                return r[d];
            };
            ...
            */
            ...
            function update(root) {
                ...
                console.log('prevOld',root,data,leaves)
                ...
                console.log(total_h,total_w,svgHeight)
                svg_tree.attr('width',total_h)
                if (svgHeight == null)
                    svgHeight = +d3.select('#partition').style('width').replace('px','')
                svg_tree.attr('viewBox', [0, 0, total_h, svgHeight].join(' '));
                ...
                let node_bar = bar_g.selectAll('.node-bar')
                    .data(d => {
                        return leaves.filter(item => {
                            return (d.data.name == item.data.설비)
                        })
                    })
                    .enter()
                    .append('g')
